...
cimport numpy as np
# import cupy as cp
from libc.math cimport sqrt, log, floor, ceil, pow, round
...
def peak_pixel_searching(np.ndarray[float64, ndim=2] f, int i_opt):
    cdef int size_f = f.shape[0]*f.shape[1]
    cdef int nx = f.shape[0]
    cdef int ny = f.shape[1]
    cdef int n0 = np.count_nonzero(~np.isnan(f))
    f_max = np.max(f)

    # remove unusual value
    max_f = np.max(f)
    if n0 == 0 or size_f == 0 or max_f == 0:
        peak_loc_x = np.nan
        peak_loc_y = np.nan
        SNR = np.nan
        MMR = np.nan
    ...
    if np.abs(i_opt) == 2:
        ...
        if ih == 0:
            ...
            if g[ih + 1] != 0 and g[ih] != 0 and g[ih - 1] != 0:
                ix_subpeak = ih - 0.5 * (np.log(g[ih + 1]) - np.log(g[ih - 1])) / \
                             (np.log(g[ih + 1]) - 2 * np.log(g[ih]) + np.log(g[ih - 1]))
            else:
                ix_subpeak = np.nan
        ...
        if ih == 0:
            ...
            if g[ih + 1] != 0 and g[ih] != 0 and g[ih-1] != 0:
                iy_subpeak = ih-0.5*(np.log(g[ih+1])-np.log(g[ih-1]))\
                             /(np.log(g[ih+1])-2*np.log(g[ih])+np.log(g[ih-1]))
            else:
                iy_subpeak = np.nan
    ...
    peak_1 = h
    g = f
    dia_peak = np.around(np.sqrt(3 * nx) / 2.5)
    lx1 = ip_x0-dia_peak
    lx2 = ip_x0+dia_peak
    ly1 = ip_y0-dia_peak
    ly2 = ip_y0+dia_peak
...
@cython.wraparound(False)
@cython.cdivision(True)
def position_particle_image_velocimetry(int nx, int ny, int nx_pixel, int ny_pixel, float64 overlap_x, float64 overlap_y):
    # distance from the ref_image edge which is divided by window size
    cdef int n_r = 0  # 1/4, 0
    if nx_pixel%2 == 0:
    ...
    cdef float64 overlaps_x = float(floor(nx_pixel * overlap_x))
    cdef float64 overlaps_y = float(floor(ny_pixel * overlap_y))

    # starting piv process at pixel point
    # defaultpivtype = 'mqd'
    # logicalpivtype = op.eq(piv_type, defaultpivtype)
    cdef float64 start_position_x = nx_pixel * n_r
    cdef float64 start_position_y = ny_pixel * n_r
    # else:
    #     start_position_x = 1
    #     start_position_y = 1

    # getting the number of vectors
    cdef int number_horizontal = int(ceil(((nx-1*start_position_x)-(nx_pixel-overlaps_x))/(nx_pixel-overlaps_x)))+1
    cdef int number_vertical = int(ceil(((ny-1*start_position_y)-(ny_pixel-overlaps_y))/(ny_pixel-overlaps_y)))+1
...
@cython.wraparound(False)
@cython.cdivision(True)
cdef inline minimum_quadric_difference(np.ndarray[float64, ndim=2] im1, np.ndarray[float64, ndim=2] im2,
                                       int nx_pixel, int ny_pixel, float64 overlap_x, float64 overlap_y,
                                       float64 iu_max, float64 iv_max):

    # defining ratio of maximum and mean
    cdef float64 r_MMR = 1.1

    # defining ratio of the first and the second mqd peak value
    cdef float64 r_PPR = 1.05

    # set the percentage of sub window for defining the searching area
    cdef float64 percentage_search = 1/3
    ...
    cdef int size_row_t1 = im1.shape[0]
    cdef int size_column_t1 = im1.shape[1]
    cdef int size_row_t2 = im2.shape[0]
    cdef int size_column_t2 = im2.shape[1]
    ...
    loc_x, loc_y, dx_center, dy_center = position_particle_image_velocimetry(size_row_t1,
                                                                             size_column_t1,
                                                                             nx_pixel,
                                                                             ny_pixel,
                                                                             overlap_x,
    ...
    number_x = len(loc_x)
    number_y = len(loc_y)
    is_x = np.zeros((number_x, number_y), dtype='float')
    is_y = np.zeros((number_x, number_y), dtype='float')

    # main process
    for i_y in range(number_y):
        for i_x in range(number_x):
            ...
            for jy in move_distance_vertical:
                ...
                for jx in move_distance_horizontal:
                    ...
                    ky1 = iy1 + jy
                    ky2 = ky1 + ny_pixel - 1
                    if (kx1 >= 0) and (kx2 <= size_row_t1 - 1) and (ky1 >= 0) and (ky2 <= size_column_t1 - 1):
                        f2 = im2[int(kx1):int(kx2), int(ky1):int(ky2)]
                        n = size_row_t2 * size_column_t2
...
@cython.wraparound(False)
@cython.cdivision(True)
def particle_image_velocimetry(np.ndarray[float64, ndim=2] im1,
                               np.ndarray[float64, ndim=2] im2,
                               int nx_pixel,
                               int ny_pixel,
                               float64 overlap_x,
                               float64 overlap_y,
    ...
    cdef np.ndarray[float64, ndim=2] im_t1 = np.transpose(im1)
    cdef np.ndarray[float64, ndim=2] im_t2 = np.transpose(im2)
    ...
    cdef int nx_pixel_1 = np.around(nx_pixel)
    cdef int ny_pixel_1 = np.around(ny_pixel)
